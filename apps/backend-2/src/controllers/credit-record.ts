...
import {
  ...
  getLeadById,
  updateCreditRecord,
  createCreditScore as createCreditScoreQuery,
} from "../database/queries/landing";
import { PMT, PV } from "../utils/math";
import { sendErrorEmail, sendPreApplicationApprovalEmail } from "./email";
      ...
      if (run.status === "completed") {
        ...
        if (message.content[0].type === "text") {
          try {
            ...
            // Delete the credit record and ask the user to upload the files again
            await deleteCreditRecord(creditRecord.id);
            const lead = await getLeadById(creditRecord.leadId!);
            await sendErrorEmail(
              "Error al procesar los estados de cuenta. Por favor, vuelve a subir tus estados de cuenta.",
              lead[0].email!
            );
          }
        }
...
export const createCreditScore = async (
  ...
  return createCreditScoreQuery(leadId, fit, probability);
};
