...
export interface DebtAcknowledgementManParams {
  dia: string;
  mes: string;
  año: string;
  nombre: string;
...
export async function generateDebtAcknowledgementManTemplate17Submission(
  params: DebtAcknowledgementManParams,
  email: string
) {
  try {
    const payload = {
      template_id: 17, // 📌 Template 17: CONTRATO PRIVADO DE USO, CARRO NUEVO (hombre)
      submitters: [
        {
          email,
          values: {
    ...
    console.log("✅ Submission CONTRATO PRIVADO DE USO (hombre) creado:", response.data);
    return response.data;
  } catch (error: any) {
    console.error(
      "❌ Error al crear submission CONTRATO PRIVADO DE USO (hombre):",
