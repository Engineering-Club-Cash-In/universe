...
if (!hasDb) {
  ...
  const buckets = ["mora_30", "mora_60", "mora_90", "mora_120", "mora_120_plus"] as const;
  ...
  describe("getMoraByEtapaYAsesor", () => {
    ...
    it("fecha anterior a la cobertura → vacío + dataDisponibleDesde", async () => {
      const r: any = await getMoraByEtapaYAsesor({ fecha: "2000-01-01" });
