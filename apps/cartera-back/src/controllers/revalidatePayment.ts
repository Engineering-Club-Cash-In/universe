...
export async function validarPagoRegistrado({
  ...
  // Cuerpo de `develop` (rechazos tipados + installmentClosed para la
  // telemetría) dentro de la función extraída en COBROS-02, que permite
  // validar dentro de una transacción ajena. Sobreviven los dos aportes.
...
async function handleRevalidatePayment(
  ...
  try {
    ...
    const result = await withPaymentAdvisoryLock(credito_id, () =>
      db.transaction((tx) =>
        validarPagoRegistrado({ credito_id, pago_id, tx })
      )
    );
