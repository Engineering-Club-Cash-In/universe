...
export async function editarRubro(
  ...
  try {
    ...
    return await db.transaction(async (tx) => {
      ...
      if (subeMonto) {
        /**
         * `FOR UPDATE` sobre el crédito, igual que en el alta.
         *
         * Esta lectura era plana, y ahí había una asimetría: `crearRubro` sí
         * bloquea la fila antes de juzgar el estado, y acá se juzga LA MISMA
         * regla (`puedeCrearRubro`) sobre la misma consecuencia —subir el monto
         * CREA DEUDA NUEVA— pero sin bloqueo. Sin él, esta lectura puede ver
         * `ACTIVO` mientras una transición a estado terminal (por ejemplo
         * `marcarCreditoComoCaido`) commitea su `CAIDO` justo después, y el alza
         * termina escribiéndose sobre un crédito que ya no admite deuda nueva.
         * Bloquear la fila ordena las dos operaciones: o el alza ve el estado
         * viejo y la transición espera, o la transición gana y el alza ve
         * `CAIDO` y se rechaza.
         */
        const [credito] = await tx
          .select({ statusCredit: creditos.statusCredit })
