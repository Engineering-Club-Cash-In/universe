...
export const recalcularPagosCredito = async ({
  ...
  const whereConditions = and(
    ...
    // La CUOTA tiene que seguir abierta, no basta con que la fila de pago
    // esté en pagado=false. Al reversar un pago de una cuota que otros pagos
    // vivos siguen cubriendo, la cuota queda pagada=true y la fila reversada
    // en pagado=false: sin este filtro esa fila entraba al recorrido, se
    // comía un tramo de amortización de una cuota YA cobrada y corría una
    // casilla todos los restantes de las cuotas siguientes (hallazgo Codex).
    // `pagado` es nullable: NULL es cuota abierta (default false), y dejarla
    // fuera con un `eq(..., false)` a secas congelaría su proyección.
    //
    // Es lo que el comentario de arriba ya declaraba ("solo lo que AÚN NO SE
    // APLICÓ al crédito: cuotas no pagadas"): acá se cumple en el SQL.
    or(isNull(cuotas_credito.pagado), eq(cuotas_credito.pagado, false)),
    or(
