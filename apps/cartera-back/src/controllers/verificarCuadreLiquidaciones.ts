...
export async function leerCuadreLiquidaciones(periodo: string): Promise<FilaCuadre[]> {
  ...
  const resultado = await db.execute(sql`
    ...
    liq as (
      ...
      --
      -- Que un inversionista tenga dos liquidaciones en el mismo mes no ocurre:
      -- el proceso es un corte mensual y en toda la historia de la tabla no hay
      -- un solo caso. liquidateByInvestorId no lo prohíbe, así que el
      -- distinct on se queda como defensa, pero verificar las anteriores no
      -- tendría contra qué compararlas: el espejo vivo ya refleja la posterior.
