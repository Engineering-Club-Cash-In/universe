...
function leerExcel(archivo) {
  ...
  for (let i = headerRow + 1; i < data.length; i++) {
    ...
    excelMap[k] = { capital: cap.toFixed(2), interes: int.toFixed(2), iva: iva.toFixed(2), isr: isr.toFixed(2), cliente };
    totals.cap += cap;
    totals.int += int;
    totals.iva += iva;
    totals.isr += isr;
...
async function procesarInversionista(c, invId, archivo, nombre) {
  ...
  for (const db of pagos.rows) {
    ...
    const capOk = Math.abs(Number(db.abono_capital) - Number(ex.capital)) < 0.02;
    const intOk = Math.abs(Number(db.abono_interes) - Number(ex.interes)) < 0.02;
    const ivaOk = Math.abs(Number(db.abono_iva_12) - Number(ex.iva)) < 0.02;
...
async function run() {
  ...
  for (const row of invDb.rows) {
    invMap[row.nombre.toLowerCase().trim()] = row;
  }
  ...
  for (const archivo of archivos) {
    ...
    let inv = invMap[nombreArchivo.toLowerCase()];
    if (!inv) {
      // Buscar parcial
      for (const [key, val] of Object.entries(invMap)) {
        if (key.includes(nombreArchivo.toLowerCase()) || nombreArchivo.toLowerCase().includes(key)) {
          inv = val;
          break;
        }
      }
