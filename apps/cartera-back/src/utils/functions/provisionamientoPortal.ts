 ...
 * office: `auth-google` también lo alcanza, desde `/api/cartera/investor` y
 * desde el registro del portal (`/api/unified/register-external-auth`). Las dos
 * llevan `requireAuth`, pero eso no alcanza: ninguna mira el rol —no puede, el
 * portal las usa con sesiones INVESTOR normales— y el sign-up de Better Auth
 * está abierto y sin verificación de correo, así que cualquiera se fabrica una
 * sesión válida. Como las dos llegan a cartera con el MISMO token de servicio
 * ADMIN, dentro de cartera el alta que viene del portal y la de back office son
 * indistinguibles por identidad: contra ESE camino el permiso tiene que venir
 * del PAYLOAD, porque mirar quién firma no distingue nada.
 *
 * (Hasta el PR #1545 existía además `/api/unified/register-external` SIN
 * `requireAuth`, o sea un camino anónimo hasta acá. Esa ruta se retiró; el
 * guard se mantiene porque las dos que quedan siguen sin probar quién llama.)
 *
 ...
 *
 * Funciona porque el registro del portal arma un objeto FIJO con
 * `{nombre, dpi, email, creado_por_usuario_portal}` y no reenvía llaves del
 * cuerpo original, y porque `/api/cartera/investor` ya no reenvía el cuerpo
 * crudo sino una whitelist de tres campos (`buildPortalInvestorUpdate`): por
 * ninguna de las dos puede colarse `provisionar_portal` desde afuera. Sin ella no se crea cuenta, no sale
