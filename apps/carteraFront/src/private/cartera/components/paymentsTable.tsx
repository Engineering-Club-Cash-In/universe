...
export function PaymentsTable() {
  ...
  const { investors } = useCatalogs() as { investors: Investor[]; advisors: any[]; loading: boolean };
  ...
  const [selectedInv, setSelectedInv] = React.useState<PagoDataInvestor["inversionistas"]>([]);
  const [modalOpen, setModalOpen] = React.useState(false);

  const handleOpenInversionistas = (inv: PagoDataInvestor["inversionistas"]) => {
    setSelectedInv(inv);
    setModalOpen(true);
  };
    ...
    <div className="   fixed inset-0 flex flex-col items-center justify-start bg-gradient-to-br from-blue-50 to-white px-2 overflow-auto pt-8 pb-8">
      <div className="bg-blue-50 rounded-xl shadow-md p-5 w-full max-w-6xl">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-5 flex-wrap">
          ...
          {/* Nombre de Usuario */}
          <div>
            <label className="block text-blue-900 font-semibold">Nombre de Usuario</label>
            <Input
              value={usuarioNombre}
              onChange={(e) => {
          ...
          {/* Inversionista */}
          <div>
            <label className="block text-blue-900 font-semibold">Inversionista</label>
            <select
  ...
  <div className="flex flex-col gap-4">
    {pagos.map((pago, idx) => (
      ...
      >
        ...
        >
          <div>
            <p className="text-blue-800 font-bold text-lg">{pago.credito?.numeroCreditoSifco}</p>
            <p className="text-blue-700 font-semibold">{formatDate(pago.fechaPago)}</p>
          </div>
          {openIdx === idx ? (
        ...
        <div className="mt-3">
          ...
          <p className="text-blue-900 font-semibold">{pago.usuario?.nombre}</p>
        </div>
        ...
        <div className="flex gap-3 mt-3">
          ...
          <button
            onClick={() => handleOpenInversionistas(pago.inversionistas)}
            className="text-blue-700 font-semibold flex items-center gap-1 hover:text-blue-900"
          >
        ...
        <div
          className={`transition-all duration-500 overflow-hidden ${
            openIdx === idx ? "max-h-[1000px] opacity-100 mt-4" : "max-h-0 opacity-0"
          }`}
        >
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: "Crédito ID", value: pago.credito?.creditoId },
              { label: "Capital", value: formatCurrency(pago.credito?.capital) },
              { label: "Deuda Total", value: formatCurrency(pago.credito?.deudaTotal) },
              { label: "Membresías", value: formatCurrency(pago.membresias) },
              ...
              { label: "Interés", value: formatCurrency(pago.abono_interes) },
              { label: "IVA 12%", value: formatCurrency(pago.abono_iva_12) },
              { label: "Seguro", value: formatCurrency(pago.abono_seguro) },
              { label: "GPS", value: formatCurrency(pago.abono_gps) },
              pago.cuota
                ? { label: "Número de Cuota", value: pago.cuota.numeroCuota }
                : null,
              pago.cuota
                ? {
                ...
                : null,
              { label: "Observaciones", value: pago.observaciones || "—" },
            ]
              .filter(Boolean)
              .map((f: any, i) => (
                <div
                ...
                >
                  <p className="text-blue-800 text-sm font-bold">{f.label}</p>
                ...
                >
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-blue-50 border-t border-blue-200">
                    {[
                      ...
                      { label: "Otros", value: formatCurrency(pago.otros) },
                      { label: "Interés", value: formatCurrency(pago.abono_interes) },
                      { label: "Abono Capital", value: formatCurrency(pago.abono_capital) },
                      { label: "IVA 12%", value: formatCurrency(pago.abono_iva_12) },
                      { label: "Seguro", value: formatCurrency(pago.abono_seguro) },
                      { label: "GPS", value: formatCurrency(pago.abono_gps) },
                      pago.cuota
