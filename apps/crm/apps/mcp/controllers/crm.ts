import db from "../db";
import { clients, leads, opportunities, companies } from "../db/schema/crm";
import { eq, desc, asc, sum, count, and, ilike, gte, lte } from "drizzle-orm";
...
export const searchOpportunities = async (filters: {
  ...
  if (filters.expectedCloseAfter)
    conditions.push(
      gte(opportunities.expectedCloseDate, new Date(filters.expectedCloseAfter))
    );
