...
export class InfornetController {
		...
		try {
			...
			if (cache.length > 0) {
				const diasRestantes = Math.ceil(
					(cache[0].expiraEn.getTime() - new Date().getTime()) /
						(1000 * 60 * 60 * 24),
				);
	...
	private async obtenerEstudioDesdeAPI(
		...
		try {
			console.log(
				`      📄 Obteniendo estudio completo para código: ${codigoPersona}`,
			);
	...
	private async guardarEnCache(
		...
		try {
			...
			console.log(
				`         - Referencias comerciales: ${estudio.referenciasComerciales.length}`,
			);
			console.log(
				`         - Referencias judiciales: ${estudio.referenciasJudiciales.delitos.length} delitos`,
			);
			...
			console.log(
				`      ✅ Estudio guardado en caché hasta: ${expiraEn.toLocaleDateString()}`,
			);
		} catch (error) {
	...
	async analizarRiesgo(dpi: string): Promise<{
		...
		const tieneDelitosPenales =
			estudio.referenciasJudiciales.delitos.length > 0;
		console.log(
			`      ${tieneDelitosPenales ? "🚨" : "✅"} Delitos penales: ${tieneDelitosPenales ? "SÍ" : "NO"}`,
		);

		const tieneMorosidad = estudio.referenciasComerciales.some(
			(ref) =>
				ref.estado?.toUpperCase().includes("MOROSO") ||
				ref.estado?.toUpperCase().includes("VENCIDO") ||
				ref.estado?.toUpperCase().includes("MORA"),
		);
		console.log(
			`      ${tieneMorosidad ? "🚨" : "✅"} Morosidad: ${tieneMorosidad ? "SÍ" : "NO"}`,
		);
		...
		console.log(
			`      ${tienePatrimonio ? "✅" : "⚠️"} Patrimonio: ${tienePatrimonio ? "SÍ" : "NO"}`,
		);
		...
		console.log(
			`   🚨 Alertas: ${alertas.length > 0 ? alertas.join(", ") : "Ninguna"}`,
		);
	...
	async obtenerEstadisticasCache(): Promise<{
		...
		try {
			const todos = await db.select().from(infornetPersonaCache);
			...
			return {
				...
				conReferenciasComerciales: todos.filter(
					(c) => c.tieneReferenciasComerciales,
				).length,
				conReferenciasJudiciales: todos.filter(
					(c) => c.tieneReferenciasJudiciales,
				).length,
	...
	async limpiarCacheExpirado(): Promise<number> {
		try {
			const resultado = await db
				...
				.where(lte(infornetPersonaCache.expiraEn, new Date()))
				.returning();

			console.log(
				`🧹 Se eliminaron ${resultado.length} registros expirados del caché`,
			);
