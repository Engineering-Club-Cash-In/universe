...
export const infornetPersonaCache = pgTable(
  'infornet_persona_cache',
  {
    ...
    estudioCompleto: jsonb('estudio_completo').notNull().$type<EstudioPersonaJSON>(),
    
    // Campos indexados para búsquedas rápidas
    tieneReferenciasComerciales: boolean('tiene_referencias_comerciales').notNull().default(false),
    tieneReferenciasJudiciales: boolean('tiene_referencias_judiciales').notNull().default(false),
  ...
  (table) => ({
    ...
    codigoPersonaIdx: index('infornet_persona_codigo_idx').on(table.codigoPersona),
