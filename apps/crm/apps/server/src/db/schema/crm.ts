import { pgTable, text, timestamp, integer, decimal, boolean, pgEnum, uuid } from "drizzle-orm/pg-core";
import { user } from "./auth";

// Enums
export const leadStatusEnum = pgEnum('lead_status', ['new', 'contacted', 'qualified', 'unqualified', 'converted']);
export const leadSourceEnum = pgEnum('lead_source', ['website', 'referral', 'cold_call', 'email', 'social_media', 'event', 'other']);
export const opportunityStatusEnum = pgEnum('opportunity_status', ['open', 'won', 'lost', 'on_hold']);
export const clientStatusEnum = pgEnum('client_status', ['active', 'inactive', 'churned']);
...
export const opportunities = pgTable("opportunities", {
  ...
  stageId: uuid("stage_id").notNull().references(() => salesStages.id),
