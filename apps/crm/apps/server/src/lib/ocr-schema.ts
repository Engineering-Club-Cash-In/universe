...
export const vehicleRegistrationOCRSchema = z.object({
	// Basic document info
	documentNumber: z.string().optional().describe("Número de tarjeta de circulación (ej: 202506579150)"),
	uniqueCode: z.string().optional().describe("Código único identificador (ej: 2025-480937-8)"),
	validUntil: z.string().optional().describe("Válida hasta fecha (ej: 31/07/2026)"),
	...
	// Vehicle basic info
	licensePlate: z.string().optional().describe("Número de placa del vehículo (ej: P0-018LKF)"),
	vehicleType: z.string().optional().describe("Tipo de vehículo (ej: CAMIONETA)"),
	make: z.string().optional().describe("Marca del vehículo (ej: CHANGAN)"),
	line: z.string().optional().describe("Línea del vehículo (ej: COROLLA, CIVIC)"),
	...
	// Dates
	registrationDate: z.string().optional().describe("Fecha de registro del vehículo"),
	...
	extractionSuccess: z.boolean().default(false).describe("Si la extracción fue exitosa o no"),
	extractionErrors: z.array(z.string()).default([]).describe("Lista de errores o campos que no se pudieron extraer"),
});

export type VehicleRegistrationOCR = z.infer<typeof vehicleRegistrationOCRSchema>;
