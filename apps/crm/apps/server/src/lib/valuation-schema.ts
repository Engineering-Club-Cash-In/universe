...
export const vehicleValuationSchema = z.object({
	suggestedValue: z.number().describe("Valor sugerido del vehículo en Quetzales"),
	reasoning: z.string().describe("Razón detallada de la valoración"),
	marketAnalysis: z.string().describe("Análisis del mercado actual para este tipo de vehículo"),
	depreciationFactors: z.array(z.string()).describe("Factores principales de depreciación considerados"),
	confidence: z.enum(["Baja", "Media", "Alta"]).describe("Nivel de confianza en la valoración"),
	marketComparison: z.string().optional().describe("Comparación con vehículos similares en el mercado"),
	recommendedActions: z.array(z.string()).optional().describe("Recomendaciones para mejorar el valor"),
});

export type VehicleValuation = z.infer<typeof vehicleValuationSchema>;

// Helper to prepare context for AI valuation
export function prepareValuationContext(vehicleData: any, checklistItems: any[], photos: any[]) {
	const today = new Date().toLocaleDateString('es-GT');
	
	// Calculate age
	const currentYear = new Date().getFullYear();
	const vehicleAge = vehicleData.vehicleYear ? currentYear - parseInt(vehicleData.vehicleYear) : 0;

	// Identify critical issues from checklist
	const criticalIssues = checklistItems?.filter(item => item.checked && item.severity === 'critical') || [];
	const warningIssues = checklistItems?.filter(item => item.checked && item.severity === 'warning') || [];

	// Extract photo comments (only photos with valuator comments, not "sin comentarios")
	const photoComments = photos?.filter(photo => 
	...
	return {
		...
		inspectionDate: vehicleData.inspectionDate ? new Date(vehicleData.inspectionDate).toLocaleDateString('es-GT') : today,
