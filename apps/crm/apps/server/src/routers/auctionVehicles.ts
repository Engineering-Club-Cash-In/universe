...
import { db } from "../db";
import { vehicleInspections, vehiclePhotos, vehicles } from "../db/schema/vehicles";
...
getAuctions: protectedProcedure
  ...
  .handler(async ({ input }) => {
    ...
    const grouped = auctions.reduce((acc, row) => {
      ...
      if (row.inspectionId && !auction.inspections.find((i: { id: string }) => i.id === row.inspectionId)) {
      ...
      // Agrupar fotos
      if (row.photoId && !auction.photos.find((p: { id: string | null; }) => p.id === row.photoId)) {
        auction.photos.push({
          id: row.photoId,
          url: row.photoUrl,
      ...
      // Agrupar gastos
      if (row.expenseId && !auction.expenses.find((e: { id: string }) => e.id === row.expenseId)) {
