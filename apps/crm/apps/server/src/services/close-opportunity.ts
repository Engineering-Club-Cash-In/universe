	...
	setImmediate(async () => {
		...
		try {
			...
			for (const invoice of invoices) {
				...
				// Identidad lógica de esta factura: la misma para el log, para el
				// gasto y para la idempotencia de cartera.
				...
				const requestBody = {
					...
					// 🔒 Con esta clave, repetir el POST (reintento, re-cierre manual,
					// respuesta perdida) NO emite una segunda factura en SAT: cartera
					// devuelve la que ya emitió.
				...
				try {
					...
					successCount++;
					console.log(
						response?.reutilizada
							? `[CloseOpportunity] ↺ Invoice "${invoice.name}" ya existía (idempotency_key), no se emitió otra`
					...
					// antes de re-facturar a mano (re-cerrar es seguro: la
					// idempotency_key devuelve la factura existente).
					await closeInvoiceSyncOperation(logId, {
						status: "error",
