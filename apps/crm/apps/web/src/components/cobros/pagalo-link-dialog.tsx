...
export function PagaloLinkDialog({
	...
	const credit = useQuery({
		...
		enabled: open && !!numeroSifco,
	});
	const data = credit.data as any;
	const cuotas = useMemo(() => {
		const vencidas = [...(data?.cuotasAtrasadas ?? [])].sort(
			...
			<DialogContent className="flex max-h-[90vh] max-w-lg flex-col overflow-hidden">
				...
				{credit.isLoading ? (
					...
					<div className="space-y-3">
						<p className="text-sm">
							{reviewRequired
								? "Grupo existente en revisión. No se creó un link adicional:"
								...
								{cuotas.map((cuota: any) => (
									...
									>
										...
										<span className="text-muted-foreground text-sm">
											Capital {q(cuota.capital_restante)} · Interés{" "}
											{q(cuota.interes_restante)}
										</span>
									</Label>
								))}
							</div>
