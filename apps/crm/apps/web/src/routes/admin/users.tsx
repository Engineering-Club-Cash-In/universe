...
import { MoreHorizontal, Shield, User, Trash2, Plus, Eye, EyeOff } from "lucide-react";
...
function RouteComponent() {
  ...
  const createUserMutation = useMutation({
    mutationFn: (input: { name: string; email: string; password: string; role: 'admin' | 'sales' }) => 
                    ...
                    <createUserForm.Field name="role">
                      {(field) => (
                        <div className="space-y-2">
                          ...
                          <Select
                            value={field.state.value}
                            onValueChange={(value) => field.handleChange(value as 'admin' | 'sales')}
                          >
                            <SelectTrigger>
                  ...
                  <createUserForm.Subscribe>
                    {(state) => (
                      <Button
                        type="submit"
                        className="w-full"
                        disabled={!state.canSubmit || state.isSubmitting || createUserMutation.isPending}
                      >
                        {state.isSubmitting || createUserMutation.isPending ? "Creating..." : "Create User"}
                      </Button>
                    )}
                  </createUserForm.Subscribe>
                </form>
