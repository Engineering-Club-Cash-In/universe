...
import { Calculator, Plus, FileText, Trash2, Send, Save, Eye, Loader2 } from "lucide-react";
...
function QuoterPage() {
  ...
  const updateInsuranceCost = async (insuredAmount: number, vehicleType: string) => {
    ...
    try {
      ...
      quoterForm.setFieldValue("insuranceCost", Math.round(result.insuranceCost * 100) / 100);
      quoterForm.setFieldValue("membershipCost", Math.round(result.membershipCost * 100) / 100);
                      ...
                      <div>
                        ...
                        <Select
                          ...
                          onValueChange={(value) => {
                            ...
                            const insuredAmount = quoterForm.state.values.insuredAmount;
                            if (insuredAmount > 0) {
                        ...
                        <Input
                          ...
                          onBlur={(e) => {
                            ...
                            if (value > 0) {
                              ...
                              // Actualizar seguro y membresía
                              updateInsuranceCost(value, quoterForm.state.values.vehicleType);
                            }
                          }}
                          onChange={(e) => {
                ...
                <CardContent className="space-y-4">
                            ...
                            {quoterForm.state.values.vehicleValue > 0
                              ? (
                                  (Number(field.state.value) /
                                    Number(quoterForm.state.values.vehicleValue)) *
                                  100
                                ).toFixed(2)
                              : "0.00"}
                            % del valor
                  ...
                  <quoterForm.Field name="interestRate">
                    {(field) => (
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          <Label htmlFor={field.name}>Tasa de Interés (%)</Label>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          Q{calculatedValues.amountToFinance.toLocaleString('es-GT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <CardContent>
                <div className="grid gap-4 md:grid-cols-3">
                  <div className="text-center p-4 bg-secondary rounded-lg">
                    <p className="text-sm text-muted-foreground">Monto Total a Financiar</p>
                    <p className="text-2xl font-bold">
                      Q{calculatedValues.totalFinanced.toLocaleString('es-GT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </p>
                  </div>
                  <div className="text-center p-4 bg-primary/10 rounded-lg">
                    <p className="text-sm text-muted-foreground">Cuota Mensual</p>
                    <p className="text-3xl font-bold text-primary">
                      Q{calculatedValues.monthlyPayment.toLocaleString('es-GT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </p>
                  </div>
                  <div className="text-center p-4 bg-secondary rounded-lg">
                    <p className="text-sm text-muted-foreground">Plazo</p>
                    <p className="text-2xl font-bold">{quoterForm.state.values.termMonths} meses</p>
                  </div>
                </div>
                ...
                <div className="mt-6 flex gap-4">
                  ...
                  >
                    ...
                    {createQuotationMutation.isPending ? "Guardando..." : "Guardar Cotización"}
                  </Button>
                  <Button
                    type="button"
                    variant="outline"
            ...
            {amortizationTable.length > 0 && (
              <Card className="mt-6">
                <CardHeader>
                  ...
                  <CardDescription>
                    Detalle del pago mes a mes durante el plazo del financiamiento
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  ...
                  <TableBody>
                    {quotationsQuery.data?.map((quotation) => (
                      <TableRow key={quotation.id}>
                        ...
                        <TableCell>Q{Number(quotation.vehicleValue).toLocaleString('es-GT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
                        <TableCell className="font-medium">
                          Q{Number(quotation.monthlyPayment).toLocaleString('es-GT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </TableCell>
                        <TableCell>
                          <Badge
                            className={statusColors[quotation.status as keyof typeof statusColors]}
                          >
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-4">
            <div className="p-4 bg-secondary rounded-lg">
              <p className="text-sm text-muted-foreground">Valor del Vehículo</p>
            ...
            <div className="p-4 bg-secondary rounded-lg">
              ...
              <p className="text-xl font-bold">Q{Number(quotation.downPayment).toLocaleString('es-GT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
            </div>
            <div className="p-4 bg-secondary rounded-lg">
              <p className="text-sm text-muted-foreground">Total a Financiar</p>
              <p className="text-xl font-bold">
                Q{Number(quotation.totalFinanced).toLocaleString('es-GT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </p>
            </div>
